...
. commons/conf.sh
CONTEXT=DSPRODUCTORDERING/api/hub
. commons/curl.sh

# OPTIONS
errOption=0
OPTIND=1
while getopts "cupdgf:i:q:" option
do
	case $option in
		c)  CREATE=OK
            ;;
...
# DELETE
if [ -n "$DELETE" ]; then
    echo "WARN: Delete all events ? ctrl+c to break" >&2
    wait
    delete "event"
    exit 2
fi
